...
如需配置后续在线 OCR 能力，可以右键浏览器工具栏里的插件图标，选择“配置 OCR”。
...
识别流程是：优先尝试浏览器内置 `BarcodeDetector`，不可用或识别失败时自动使用本地 `jsQR`。如果直接识别失败，会继续生成补白边、放大、对比度增强和二值化后的图像版本进行重试。整个过程都在浏览器本地完成，不需要服务端接口。
